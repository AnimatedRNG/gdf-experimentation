...
class Shader:
    ...
    def createShader(self, strings, type):
        ...
        pyglet.gl.glShaderSource(shader, count, cffi.cast(
            cffi.pointer(src), cffi.POINTER(cffi.POINTER(cffi.c_char))), None)
        ...
        pyglet.gl.glGetShaderiv(
            shader, pyglet.gl.GL_COMPILE_STATUS, cffi.byref(temp))

        # if compilation failed, print the log
        if not temp:
            # retrieve the log length
            pyglet.gl.glGetShaderiv(
                shader, pyglet.gl.GL_INFO_LOG_LENGTH, cffi.byref(temp))
    ...
    def link(self):
        ...
        pyglet.gl.glGetProgramiv(
            self.handle, pyglet.gl.GL_LINK_STATUS, cffi.byref(temp))
        # if linking failed, print the log
        if not temp:
            #   retrieve the log length
            pyglet.gl.glGetProgramiv(
                self.handle, pyglet.gl.GL_INFO_LOG_LENGTH, cffi.byref(temp))
            # create a buffer for the log
            buffer = cffi.create_string_buffer(temp.value)
            # retrieve the log text
    ...
    def enumerate_attributes(self):
        param = cffi.c_int(0)
        pyglet.gl.glGetProgramiv(
            self.handle, pyglet.gl.GL_ACTIVE_ATTRIBUTES, cffi.byref(param))
        self.attributeCount = param.value
        print("{} attributes ".format(self.attributeCount))
        ...
        for i in range(0, self.attributeCount):
            ...
            self.attributes[cname.value] = \
                {"location": location, "info": (
                    attrib_type.value, attrib_size.value)}
        return
